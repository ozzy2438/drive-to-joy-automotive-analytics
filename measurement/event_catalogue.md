...
| `test_drive_start` | Form begins | `vehicle_model`, `dealer_id`, `form_type` | Form funnel |
| `form_error` | Validation issue | `form_type`, `form_field`, `form_error_type` | UX friction |
| `test_drive_submit` | Form submits | `vehicle_model`, `dealer_id`, `lead_id_hash`, `form_type` | Primary web conversion |
| `quote_submit` | Quote form submits | `vehicle_model`, `dealer_id`, `lead_id_hash`, `form_type` | Primary web conversion |
| `experiment_exposure` | Variant shown | `experiment_id`, `variant_id` | Experiment analysis |
| `personalisation_exposure` | Experience shown | `audience_id`, `experience_id`, `holdout_flag` | Personalisation analysis |
